...
from llm_client.providers.types import CompletionResult, Message, ToolCall, Usage
...
class TestToolValidation:
    ...
    def test_invalid_name_pattern(self):
        ...
        assert not result.valid
        # Jsonschema says "does not match"
        assert any("match" in e.lower() or "failed" in e.lower() for e in result.errors)
...
class TestSchemaValidation:
    ...
    def test_invalid_required_type(self):
        """Test invalid required type (must be array)."""
        schema = {
            "type": "object",
            "required": "not-a-list",
        }
