...
class TestAgentBasics:
    ...
    async def test_agent_single_turn(self, mock_provider, mock_completion_result):
        ...
        provider = mock_provider(responses=[mock_completion_result(content="Hello! How can I help?")])
    ...
    async def test_agent_with_tool_call(
        ...
        tool_call = mock_tool_call(name="test_tool", arguments='{"message": "test"}')
    ...
    async def test_agent_max_turns(self, mock_provider, mock_completion_result, mock_tool_call):
        ...
        provider = mock_provider(
            responses=[mock_completion_result(content=None, tool_calls=[tool_call]) for _ in range(10)]
        )
    ...
    async def test_agent_tool_error_handling(
        ...
        tool_call = ToolCall(id="call_err", name="error_tool", arguments='{"should_fail": true}')
...
class TestAgentStreaming:
    ...
    async def test_agent_stream_simple(self, mock_provider, mock_completion_result):
        ...
        async def mock_stream(messages, **kwargs):
            ...
            yield StreamEvent(type=StreamEventType.DONE, data=mock_completion_result(content="Hello there!"))
