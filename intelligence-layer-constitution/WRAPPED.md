...
## 9) Demos (what’s executable today)

The `demo/` folder is a reference implementation of “schemas + manifests + plan templates” in action.

- `demo/kernel_email_review.py`: loads all schemas/manifests, resolves bindings, runs steps, emits SSE-shaped events.
- `demo/run_email_review_demo.py`: deterministic run (no LLM) showing `action_required` and resume.
- `demo/kernel_ai_outreach.py` + `demo/run_ai_outreach_chat.py`: adds a switchboard and LLM-backed operators (offline `MockLLM` by default), emits `token_delta`, pauses at apply gate.
- `demo/fastapi_email_review_app.py`: optional FastAPI wrapper with SSE streaming.

This is not the production Kernel—it's a deliberately small “make the architecture tangible” harness.

---

## 10) Data layer + infra (what v1 is designed for)
...
## 11) Credits/budget enforcement (the “economic policy” layer)
...
## 12) What’s missing (why this repo is “constitution + contracts”, not the service)

Per `STATUS.md`, the repo is ahead in **design contracts** and behind in **production implementation**.

Major missing pieces (by design, not an accident):

- the real production Kernel API + persistence + workers (the demo shows the shape)
- Postgres migrations/DDL actually applied (DDL exists as design in `DATA-STRUCTURE.md`)
- policy engine implementation + policy decision ledger wiring
- platform adapters for:
  - session validation
  - credits/quota reservation/settlement
  - patch application execution + receipt storage
- real operator implementations and sandboxed credentials/runtime separation
- observability wiring (OTel, logs/metrics/traces) and hard SLOs

---

## 13) How you extend this system (the “contracts-first” workflow)
...
## 14) Reading order (so you don’t drown)

If you want the “wide picture” fast:

1) `CONSTITUTION.md` (the law)  
2) `STATUS.md` (what exists vs what’s planned)  
3) `FEATURE-DEVELOPMENT-WALKTHROUGH.md` (how the repo artifacts connect)  
4) `API-RUNTIME-DESIGN.md` + `DATA-STRUCTURE.md` (what the production service will look like)  
5) `demo/README.md` then run the demos to feel the runtime model  
