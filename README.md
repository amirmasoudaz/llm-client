...
> Build and recipes guide: see
> [`docs/llm-client-build-and-recipes-guide.md`](docs/llm-client-build-and-recipes-guide.md).
...
> [`docs/llm-client-extraction-matrix.md`](docs/llm-client-extraction-matrix.md),
> [`docs/llm-client-adoption-notes-agent-runtime.md`](docs/llm-client-adoption-notes-agent-runtime.md),
> [`docs/llm-client-adoption-notes-intelligence-layer.md`](docs/llm-client-adoption-notes-intelligence-layer.md),
...
- `create_vector_store_and_poll(...)` now supports typed `files=[ResponsesVectorStoreFileSpec(...)]` provisioning in addition to raw `file_ids`, so a fresh hosted retrieval store can be created and fully ingested in one workflow call.
- `RealtimeConnection` now includes first-class client-event helpers for `create_text_message(...)`, `append_input_audio_chunks(...)`, `commit_audio_and_create_response(...)`, `disable_vad(...)`, `send_audio_turn(...)`, `update_session_tools(...)`, `create_response_with_tools(...)`, realtime `mcp_approval_response` creation, `conversation.item.retrieve`, `conversation.item.delete`, `conversation.item.truncate`, `response.cancel`, and typed receive-side helpers via `recv_event()` / `recv_until_type(...)`, `wait_for_mcp_tool_listing(...)`, plus collected output via `collect_response_output(...)`.
...
[`docs/llm-client-tool-creation-guide.md`](docs/llm-client-tool-creation-guide.md).
