...
class OpenAIProvider(BaseProvider):
    ...
    def _check_reasoning_params(
        ...
        if not self._model.reasoning_model:
            raise ValueError(
                "Model does not support reasoning, but reasoning parameters were provided."
            )
        ...
        if has_reasoning_effort:
            ...
            if effort is not None and reff is not None and reff != effort:
                raise ValueError(
                    "Provide only one of `reasoning` or `reasoning_effort`, or ensure they match."
                )
            effort = reff if effort is None else effort
        
        if effort not in self._model.reasoning_efforts:
            raise ValueError(
                f"Invalid reasoning effort. Choose from: {self._model.reasoning_efforts}"
            )
    ...
    def _normalize_response_format(
        ...
        if response_format == "json_object":
            ...
            if "json" not in str(messages).lower():
                raise ValueError(
                    "Context doesn't contain 'json' keyword which is required for JSON mode."
                )
    ...
    async def stream(
        ...
        **kwargs: Any,
        ...
        yield StreamEvent(
            ...
            data={"model": self.model_name, "stream": True}
        )
        ...
        async with self.limiter.limit(tokens=input_tokens, requests=1) as limit_ctx:
            try:
                ...
                yield StreamEvent(
                    ...
                    data={"status": 500, "error": str(e.__cause__)}
                )
            except openai.RateLimitError as e:
                yield StreamEvent(
                    type=StreamEventType.ERROR,
                ...
                yield StreamEvent(
                    type=StreamEventType.ERROR,
                    data={"status": e.status_code, "error": str(e.response)}
                )
        ...
        async with self.limiter.limit(tokens=input_tokens, requests=1):
            try:
                ...
                if encoding_format == "base64":
                    embeddings = [
                        np.frombuffer(base64.b64decode(emb), dtype=np.float32).tolist()
                        if isinstance(emb, str) else emb
    ...
    def _result_to_cache(self, result: CompletionResult, params: Dict[str, Any]) -> Dict[str, Any]:
        """Convert CompletionResult to cache-friendly dict."""
        return {
            "params": params,
    ...
    def _cached_to_result(self, cached: Dict[str, Any]) -> CompletionResult:
        """Convert cached dict back to CompletionResult."""
        tool_calls = None
