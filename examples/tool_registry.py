...
async def search_database(
    ...
    results = [{"id": i, "table": table, "match": query, "deleted": False} for i in range(min(limit, 3))]
...
async def main():
    ...
    result = await registry.execute("search_database", '{"query": "john", "table": "users", "limit": 5}')
    ...
    result = await registry.execute("get_weather", '{"city": "Tokyo", "units": "fahrenheit"}')
    print(f"  Success: {result.success}")
    print(f"  Result: {result.content}")
    ...
    result = await provider.complete(query, tools=registry.tools, tool_choice="auto")
